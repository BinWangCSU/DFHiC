...
# the code is provided: https://github.com/kimmo1019/hicGAN
...
do
	number=$(expr $idx + 549)
	# number=$(expr $idx + 545)
	wget -c -P "$DPATH/$1" ftp://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1551nnn/GSM1551`printf "%03d" $number`/suppl/GSM1551`printf "%03d" $number`_HIC`printf "%03d" $idx`_merged_nodups.txt.gz
done
